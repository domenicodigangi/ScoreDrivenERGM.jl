...
UMdgp,dgpdegs = linSpacedPar(fooSdErgm1,N; NgroupsW = GW)
Bdgp = 0.9*ones(GBA)
Adgp = 0.1*ones(GBA)
Wdgp = UMdgp.*(1-Bdgp[groupsInds[2]])
scoreScal = "FISHER-EWMA"#""#
Mod = SdErgm1(ones(2,2),[Wdgp , Bdgp, Adgp],groupsInds,scoreScal)
...
estOut = estimate(Mod)
Mod_est = SdErgm1(Mod.obsT,estOut[1],groupsInd,scoreScal)
