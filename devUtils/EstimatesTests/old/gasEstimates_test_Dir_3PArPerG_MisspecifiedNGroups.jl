...
groupsIndsDgp = [[DynNets.distributeAinVecN(Array{Int}(1:GWdgp),2N)];
                    [DynNets.distributeAinVecN(Array{Int}(1:GBAdgp),GWdgp)]]
groupsIndsEst = [[DynNets.distributeAinVecN(Array{Int}(1:GWest),2N)];
...
realUm = DynNets.linSpacedFitnesses(N;Ngroups = GWdgp,mode = "DIRECTED")
...
allObs = SharedArray{Int16,4}(2N,Tvals[end],N_ind,N_est)
...
#  n=1
        ...
        Model2Sample = DynNets.GasNetModelDBin1(ones(T,2N),[sim_W,sim_B,sim_A],
                                                    groupsIndsDgp)
        Y_T,fit_T  =  DynNets.sampl(Model2Sample,T)
        sampleDegsT = [squeeze(sum(Y_T,2),2) squeeze(sum(Y_T,3),3)]
        allObs[:,1:T,ind,n] = sampleDegsT'
        Model2Est =  DynNets.GasNetModelDBin1(sampleDegsT,[zeros(GWest),zeros(GBAest),zeros(GBAest)],
                                                    groupsIndsEst)
        #estimate
    ...
    file_name = "EstTest_Dir_$(N)_$(GWdgp)_$(GBAdgp)_$(Tvals)_$(N_est)_$(useStartVal)_Miss_$(GWest)_$(GBAest).jld"
